...
public class GetUrlInfo {
	...
	public Bitmap getWebPic (String imageUrl){
		Bitmap mBitmap = null;
		InputStream bytedata = getWebData(imageUrl);
		mBitmap = BitmapFactory.decodeStream(bytedata);
		return mBitmap;
	}
	/**
	 * 获取远程数据
	 */
	public  InputStream getWebData (String weburl) {
		Log.v("Path", weburl);
		InputStream bytedata = null;
		try {
			URL url = new URL(weburl);
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			bytedata = conn.getInputStream();
